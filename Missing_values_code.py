...
r1=[]
'''
#taken the number of rows
N=int(input())

#build the dataframve by recursively
for i in range(N+1):
...
'''
df1=pd.read_csv('df.csv',header=0)
N=df1.shape[0]
...
model11=linear_model.LinearRegression(normalize=True)
model11.fit(train.loc[:,train.columns!='tmax'],train.loc[:,'tmax'])

for i,j in enumerate(model11.predict(test1)):
    r1.append((test1.index[i],round(j,1)))
    
model12 = linear_model.LinearRegression(normalize=True)
model12.fit(train.loc[:,train.columns != 'tmin'],train.loc[:,'tmin'])

for i,j in enumerate(model12.predict(test2)):
